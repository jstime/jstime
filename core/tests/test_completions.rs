...
mod completions {
    ...
    #[test]
    fn get_property_names_for_lexical_binding_object() {
        let _setup_guard = common::setup();
        let options = jstime::Options::default();
        let mut jstime = jstime::JSTime::new(options);

        jstime
            .run_script("const lexicalObj = { foo: 1, bar: 2 };", "test")
            .unwrap();

        let props = jstime.get_property_names("lexicalObj");
        assert!(props.contains(&"foo".to_string()));
        assert!(props.contains(&"bar".to_string()));
    }
    ...
    fn get_property_names_allows_safe_expressions() {
        ...
        // Safe expressions with alphanumeric, underscore, dollar sign, and dots should work
        ...
        jstime
            .run_script("const $myObject = { test: 1 };", "test")
            .unwrap();
        assert!(!jstime.get_property_names("$myObject").is_empty());
    }
}
